...
class ScheduleZBVZeroBubble(PipelineScheduleMulti):
    ...
    def _get_zbv_schedule(
        ...
        compute_schedules = {}
        category_map = {
            _ComputationType.FORWARD: 0,
            _ComputationType.BACKWARD: 1,
            _ComputationType.WEIGHT: 2,
        }
        chunk_0 = 0
        chunk_1 = 1
        def get_compute_schedule(pipeline_parallel_size, num_microbatches):
            ...
            for i in range(pipeline_parallel_size):
                temp = {}
                temp[_ComputationType.FORWARD] = [0] * 2
                temp[_ComputationType.BACKWARD] = [0] * 2
                temp[_ComputationType.WEIGHT] = [0] * 2
            ...
            def put_w(rank):
                ...
                put(_ComputationType.WEIGHT, chunk_, rank)
            ...
            def put(op, chunk, rank, assert_cnt=True):
                ...
                if chunk == chunk_1 and op in (_ComputationType.FORWARD, _ComputationType.BACKWARD):
                ...
                if chunk == chunk_0 and op in (_ComputationType.FORWARD, _ComputationType.BACKWARD):
                    if rank > 0:
                        _fa_id = get_id(op, chunk, rank - 1, microbatch_id=_op_count)
                        assert end_time[_fa_id] >= 0, f"{category}, {chunk}, {rank}, {_op_count}"
                        _cur_time = max(_cur_time, end_time[_fa_id] + 1)
                _id = get_id(op, chunk, rank, microbatch_id=_op_count)
                if ops_count[rank][_ComputationType.FORWARD][chunk_0] > 0:
                ...
                temp_chunk = chunk if op == _ComputationType.FORWARD else 1 - chunk
                schedule[rank].append((_op_count, op, temp_chunk))
                if op == _ComputationType.BACKWARD:
                    pending_w[rank].append((_ComputationType.WEIGHT, chunk, _op_count))
            ...
            for cur_rank in range(pipeline_parallel_size):
                put(_ComputationType.FORWARD, chunk_0, cur_rank)
            ...
            for cur_rank in range(pipeline_parallel_size - 1, -1, -1):
                if cur_rank == pipeline_parallel_size - 1:
                    put(_ComputationType.FORWARD, chunk_1, cur_rank)
                    continue
                tmp = end_time[get_id(_ComputationType.FORWARD, chunk_1, cur_rank + 1, microbatch_id=0)]
                while (
                    cur_time[cur_rank] + 1 <= tmp
                    and ops_count[cur_rank][_ComputationType.FORWARD][chunk_0] < num_microbatches
                ):
                    for prev_rank in range(cur_rank + 1):
                        put(_ComputationType.FORWARD, chunk_0, prev_rank)
                put(_ComputationType.FORWARD, chunk_1, cur_rank)
            ...
            for current_rank in range(1, pipeline_parallel_size):
                ...
                while (
                    ops_count[current_rank][_ComputationType.FORWARD][chunk_0]
                    + ops_count[current_rank][_ComputationType.FORWARD][chunk_1]
                    < ops_count[previous_rank][_ComputationType.FORWARD][chunk_0]
                    + ops_count[previous_rank][_ComputationType.FORWARD][chunk_1]
                    or ops_count[current_rank][_ComputationType.FORWARD][chunk_1]
                    <= ops_count[previous_rank][_ComputationType.FORWARD][chunk_1]
                    < num_microbatches
                ):
                    # Schedule forward operations for backward pipeline stages
                    for stage in range(pipeline_parallel_size - 1, previous_rank, -1):
                        if ops_count[stage][_ComputationType.FORWARD][iter_chunk_] < num_microbatches:
                            put(_ComputationType.FORWARD, iter_chunk_, stage)
            ...
            for _ in range(2 * num_microbatches):
                ...
                chunk_1_ready_ranks = []
                for rank in range(pipeline_parallel_size):
                    chunk_0_ops = ops_count[rank][_ComputationType.BACKWARD][chunk_0]
                    chunk_1_ops = ops_count[rank][_ComputationType.BACKWARD][chunk_1]
                    if chunk_1_ops >= chunk_0_ops:
                        ...
                        dependency_id = get_id(
                            _ComputationType.BACKWARD, chunk_1, rank + 1, microbatch_id=chunk_1_ops
                        )
                ...
                for rank, chunk in scheduled_ranks:
                    ...
                    if chunk == chunk_1 and rank < pipeline_parallel_size - 1:
                        dependency_id = get_id(
                            _ComputationType.BACKWARD, chunk_1, rank + 1, microbatch_id=ops_count[rank][_ComputationType.BACKWARD][chunk_1]
                        )
                    if chunk == chunk_0 and rank > 0:
                        dependency_id = get_id(
                            _ComputationType.BACKWARD, chunk_0, rank - 1, microbatch_id=ops_count[rank][_ComputationType.BACKWARD][chunk_0]
                        )
                    ...
                    put(_ComputationType.BACKWARD, chunk, rank)

                # Schedule forward operations for each pipeline rank
                for rank in range(pipeline_parallel_size):
                    # Skip if all forward operations for chunk_1 are completed
                    if ops_count[rank][_ComputationType.FORWARD][chunk_1] >= num_microbatches:
                        continue

                    # Determine which chunk to prioritize for the forward operation
                    put_item = None
                    forward_chunk_1_count = ops_count[rank][_ComputationType.FORWARD][chunk_1]
                    forward_chunk_0_count = ops_count[rank][_ComputationType.FORWARD][chunk_0]
                    ...
                    if forward_chunk_1_count >= forward_chunk_0_count:
                        ...
                        dep_id_chunk_1 = get_id(
                            _ComputationType.FORWARD, chunk_1, rank + 1, microbatch_id=forward_chunk_1_count
                        )
                        if end_time[dep_id_chunk_1] >= 0:
                            ...
                            if rank == 0:
                                ...
                                dep_id_chunk_0 = get_id(
                                    _ComputationType.FORWARD, chunk_0, rank - 1, microbatch_id=forward_chunk_0_count
                                )
                    ...
                    put(_ComputationType.FORWARD, put_item, rank)
